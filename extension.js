...
// 获取coding时长
function getCodeActivity() {
  api_key = read_api_key()
  abc = `https://api.wakatime.com/api/v1/users/current/statusbar/today?api_key=${api_key}`
  request(abc,
    function(error, response, body) {
      if (!error && response.statusCode == 200) {
        if (debug) console.log('get code activity success.')
  ...
  // 每5分钟请求一次
  setTimeout(getCodeActivity, 300000)
}
// 在状态栏显示时间
function displayTime() {
  if (temp_str) {
    hx.window.setStatusBarMessage(temp_str, 10000, 'info')
  }
...
function activate(context) {
  ...
  hx.workspace.onDidSaveTextDocument(function(document) {
    ...
    handleAction(true)
    // setInterval(hx.window.setStatusBarMessage(temp_str), 10000)
  })
